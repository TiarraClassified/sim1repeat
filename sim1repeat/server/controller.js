module.exports = {
    ...
    addBin: (req, res) => {
        ...
        db.get_bin([req.params.id[0], req.params.id[1]]).then(inventory => {
            if(inventory.length === 0) {
                // console.log(req.body.name, req.params.id[0], req.params.id[1], req.body.image, Number(req.body.price));
                db.add_bin([req.body.name, req.params.id[0], req.params.id[1], req.body.image, Number(req.body.price)]).then(inventory => {
                    console.log(inventory[0]);
                    res.status(200).send(inventory[0])
                }).catch(e=>console.log(e, 'got bin'))
            } else {
                res.status(200).send('Bin already taken');
            }
        }).catch(e=>console.log(e, 'did not get bin'))
    }
}